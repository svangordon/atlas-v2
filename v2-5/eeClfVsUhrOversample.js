...
function classifyZone(classificationZone) {
  ...
  // Split the training / testing data before sampling to precent any
  //  co-located training and testing poitns
  labelLocations = ee.FeatureCollection(labelLocations).randomColumn('trainTestSplit', 0)

  var trainingSize = 0.7
  var trainingLocations = labelLocations.filter(ee.Filter.lt('trainTestSplit', trainingSize))
  var testingLocations = labelLocations.filter(ee.Filter.gte('trainTestSplit', trainingSize))


  var trainingData = getTrainingInputs(trainingImage, labelImage, trainingLocations)
    .filter(ee.Filter.notNull(trainingBands))

  var testingData = getTrainingInputs(trainingImage, labelImage, testingLocations)
    .filter(ee.Filter.notNull(trainingBands))

  /*
    Oversample minority classes to give an even split of classes in the training data
  */


  // Get the number of classes in our expanded area
  var numberOfClasses = trainingData.aggregate_count_distinct('b1')
  // Get the number of samples in the training set
  var samplesInTrainingSet = trainingData.size()
  ...
  var algorithm = ee.Classifier.randomForest(20)
    .train(trainingData, classBand, trainingBands)
  var accuracy = testingData.classify(algorithm).errorMatrix(classBand, 'classification').accuracy()
  //
  // var output = ee.Feature(null, {
  //   accuracy: testingAccuracy,
  //   algorithm: algorithm,
  ...
  // var trainingResult = trainAlgorithm(data, trainingBands, classBand)
