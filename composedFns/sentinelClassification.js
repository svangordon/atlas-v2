...
var maskSentinel = cloud_masks.sentinel2()

var geometry = /* color: #98ff00 */ee.Geometry.Polygon(
        [[[-1.6369385551918185, 5.861954466806234],
          [-1.3733030193983495, 5.851026927937325],
          [-1.3513339819999146, 6.0258404396698415],
          [-1.6204607734357523, 6.053149871532996]]]);
...
function getTrainingInputs(basemap, labels, samplingGeometry, samplingScale) {
  ...
  samplingScale = samplingScale || 20;
...
function getLandsatImage(geometry, year, landsatCollection) {
  var startDate = ee.Date.fromYMD(year, 11, 15)
  var endDate = ee.Date.fromYMD(ee.Number(year).add(1), 3, 15)
...
function classifyZone(classificationZone) {
  ...
  var trainingYear = 2015
  var classificationYear = 2015
  ...
  var sentinelBands = ['B1', 'B2', 'B3', 'B4', 'B8', 'B9', 'B11', 'B12']
  var demBands = ['aspect', 'elevation', 'hillshade', 'slope']
  var trainingBands = sentinelBands
  var classBand = 'b1'
  ...
  var trainingImageCollection = sentinelCollection;
...
var geometry = ecowas.filter(ee.Filter.eq('NAME', 'Ghana'))
  .geometry()
  .convexHull()
// geometry = geometry2
