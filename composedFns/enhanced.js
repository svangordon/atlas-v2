...
function visualizeClassification(classificationImage) {
  // Cast classificationImage to Image
  classificationImage = ee.Image(classificationImage)
...
function trainAlgorithm(inputData, trainingBands) {
  ...
  var trainingSize = trainingData.size()
  var testingSize = testingData.size()
  var minSize = trainingSize.min(testingSize)
...
function classifyZone(classificationZone) {
  ...
  // Create buffered training basemap
  // var bufferedZone = classificationZone.buffer(56000)
  ...
  var minSize = trainingResult.get('minSize')

  return ee.Algorithms.If(
    minSize,
    trainingImage.classify(ee.Classifier(algorithm))
      .set('accuracy', accuracy),
    ee.Image(0).set('accuracy', 0)
  )
...
var geometry = ecowas.filter(ee.Filter.eq('NAME', 'C�te d\'Ivoire'))
  .geometry()
  .convexHull()
  .transform(atlas_2000.projection(), 10)
...
var zones = getZonesBoundaries(geometry)
  ...
  .map(function (zone) {
    var samplingPoints = getLabelLocations(zone, atlas_2000)
    var numberOfPoints = atlas_2000.sampleRegions({
          collection: samplingPoints,
          scale: 2000,
          projection: atlas_2000.projection()
        }).size()
    return ee.Algorithms.If(
      ee.Algorithms.IsEqual(ee.Number(0), numberOfPoints),
      null,
      zone
    )
  }, true)
...
// Map.addLayer(visualizeClassification(classifiedImage), {}, 'RGB Visualized')
